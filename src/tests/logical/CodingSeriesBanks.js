...
const codingPatterns=Array.from({length:25},(_,p)=>v=>{
 ...
 const candidates=typeof correct==="string"&&/^\d+$/.test(correct)?[String(+correct+v),String(Math.max(1,+correct-v)),String(+correct+2*v),String(+correct+v+3)]:[shift(word,k+1),[...correct].reverse().join(""),shift(word,Math.max(1,k-1)),mirror(word),[...word].reverse().join("")];
 const distractors=[...new Set(candidates)].filter(option=>option!==correct).slice(0,3);
...
const seriesPatterns=Array.from({length:25},(_,p)=>v=>{
 ...
 const numeric=typeof correct==="number", letterAt=offset=>String.fromCharCode(65+(correct.charCodeAt(0)-65+offset+26)%26), distractors=numeric?[...new Set([correct+v,correct-v,correct+(family+2),correct+v+3,Math.max(0,correct-v-2)].map(String))].filter(option=>option!==String(correct)).slice(0,3):[letterAt(1),letterAt(-1),letterAt(2)];
