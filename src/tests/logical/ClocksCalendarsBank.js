...
const patterns=Array.from({length:25},(_,p)=>v=>{
 ...
 else if(family===1){const h=1+((band+v)%10),m=5*(1+((band+2*v)%10));let total=11*60-(h*60+m);total=((total%(12*60))+(12*60))%(12*60);const mh=total%60,hh=Math.floor(total/60)||12;correct=`${hh}:${String(mh).padStart(2,"0")}`;prompt=`A mirror is placed in front of a standard 12-hour analogue clock. If the actual time is ${h}:${String(m).padStart(2,"0")}, what time appears in the mirror?`;explanation=`For an ordinary mirror image, subtract the shown time from 11:60. The result is ${correct}.`;distractors=[`${h}:${String(60-m).padStart(2,"0")}`,`${12-h}:${String(m).padStart(2,"0")}`,`${hh}:${String((mh+10)%60).padStart(2,"0")}`];}
